...
>
    ...
    <div class="space-y-8">
        <x-contoso.form-section heading="{{ $isEdit ? 'Edit Order #' . $order->number : 'New Order' }}">
            <x-slot:description>
                @if ($isEdit)
                    <p class="text-sm text-gray-500">
                        Created {{ $order->created_at->diffForHumans() }} by {{ $order->creator->name }}.
                        @if ($order->updated_at->gt($order->created_at))
                            Last modified {{ $order->updated_at->diffForHumans() }}.
                        @endif
                    </p>
                @else
                    <p class="text-sm text-gray-500">Fill in the order details below. Required fields are marked with an asterisk.</p>
                @endif
            </x-slot>
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-6">
                ...
                <div class="sm:col-span-3">
                    ...
                    />
                    @error('customer_id')
                        <x-contoso.form-error>{{ $message }}</x-contoso.form-error>
                    @enderror
                </div>
                ...
                >
                    ...
                    <div class="flex gap-2">
                        @foreach ($priorityLevels as $key => $priority)
                            <button
                                type="button"
                                ...
                                @if (($isEdit ? $order->priority : 'normal') === $key)
                                    aria-pressed="true"
                                @endif
                            >
                                {{ $priority['label'] }}
        ...
        <x-contoso.form-section heading="Line Items">
            ...
            <div class="space-y-4">
                    ...
                    >
                        ...
                        <div class="sm:col-span-4">
                            ...
                            <button
                                ...
                                x-text="expanded ? 'Hide description' : 'Edit description'"
                            ></button>
                            ...
                            >
                                ...
                                @foreach ($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}">
                                        {{ $warehouse->name }}
                                        @if ($warehouse->is_primary) (Primary) @endif
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <div class="relative">
                                <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-gray-500" x-text="currency"></span>
                                <input
                                    type="number"
                ...
                <div class="flex items-center justify-between border-t border-gray-200 pt-4">
                    ...
                    @if ($isEdit && $order->lineItems->count() > 0)
                        <span class="text-xs text-gray-500">
                            {{ $order->lineItems->count() }} item(s) saved
                        </span>
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                <div x-show="showNotes" x-collapse>
                    <textarea
                        ...
                        placeholder="Notes visible only to staff members..."
                    ></textarea>
                </div>
            </div>
        </x-contoso.form-section>
        ...
        @if ($isEdit && $order->activities->isNotEmpty())
                                        ...
                                        <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                            <div>
                                                <p class="text-sm text-gray-500">
                                                    ...
                                                    @if ($activity->causer)
                                                        by <span class="font-medium text-gray-900">{{ $activity->causer->name }}</span>
                                                    @endif
                                                </p>
                                            </div>
