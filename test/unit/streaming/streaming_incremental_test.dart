...
void main() {
  group('Streaming Incremental Emission Tests (EXPECTED TO FAIL)', () {
    test(
      'Nodes should emit before stream completion',
      () async {
        ...
        Future.delayed(
          ...
          () => controller.add('<p>Second</p>'),
        );
        Future.delayed(
          Duration(milliseconds: 200),
          () => controller.add('<span>Third</span>'),
        );
        ...
        final nodes = <AstNode>[];
        await for (final node in streamingParser.parseStreaming(
          controller.stream,
        )) {
    ...
    test(
      'Each top-level element should be available as closing tag arrives',
      () async {
        final streamingParser = StreamingParser();
    ...
    test('Chunk split mid-token should still work', () async {
      ...
      expect(nodes, isNotEmpty, reason: 'Should handle chunks split mid-token');

      final divElements = nodes
          .expand(
            (n) => n is HtmlElementNode
                ? [n]
                : (n as DocumentNode).children.whereType<HtmlElementNode>(),
          )
      ...
      final textContent = div.children
          .whereType<TextNode>()
          .map((t) => t.content)
          .join();
      ...
      final htmlElements = nodes
          .expand(
            (n) => n is HtmlElementNode
                ? [n]
                : (n is DocumentNode
                      ? n.children.whereType<HtmlElementNode>()
                      : <HtmlElementNode>[]),
          )
          .toList();
    ...
    test('Streaming should maintain state across chunks', () async {
      ...
      if (divElements.isNotEmpty) {
        ...
        final sections = div.children.whereType<HtmlElementNode>().where(
          (e) => e.tagName == 'section',
        );
        ...
        if (sections.isNotEmpty) {
          ...
          final articles = section.children.whereType<HtmlElementNode>().where(
            (e) => e.tagName == 'article',
          );
